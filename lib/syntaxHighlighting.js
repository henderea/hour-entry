...
function processLine(s, state) {
    s.eachChar(c => {
        ...
        if(
            s.onChar(' ') && !s.hasPieces && s.currentTextMatches(/^=$/) && !s.remainingTextMatches(/^=*\s*$/) && !state.atStart && !state.atEnd &&
            (/^\s*$/.test(state.previousLine) || (state.previousLineState.hasPieces && /^=+\s?/.test(state.previousLineState.pieces[0].text))) &&
    ...
    s.cleanup();
    if(state.boxStart >= 0 && /^=?\s*$/.test(s.line) && (state.atEnd || !(/^=+\s*/.test(state.nextLine)))) {
        state.processBox();
    } else if(!(/^=*\s*$/.test(s.line) || /^=\s/.test(s.line))) {
        state.boxStart = -1;
    }
}
...
function lineStateFormatter(s, colors) {
    let parts = s.mapPieces((text, type, trimmedText, p) => {
        ...
        console.log(p, text, color, colors, extra);
